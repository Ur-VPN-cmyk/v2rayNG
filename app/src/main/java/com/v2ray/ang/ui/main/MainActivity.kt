...
class MainActivity : HelperBaseComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        AppPresetConfig.ensureGlobalRoutingDefaults(this)
        if (MmkvManager.decodeServerList().isEmpty()) {
            AngConfigManager.importBatchConfig(AppPresetConfig.DEFAULT_VLESS_URI, "")
            if (MmkvManager.decodeSubsList().isEmpty()) {
                val subItem = SubscriptionItem().apply {
                    remarks = "Backup Subscription"
                    url = AppPresetConfig.DEFAULT_SUB_URL
                }
                MmkvManager.encodeSubscription(Utils.getUuid(), subItem)
            }
            val updatedList = MmkvManager.decodeServerList()
            if (updatedList.isNotEmpty()) {
                MmkvManager.selectServerId(updatedList.first())
            }
        }
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        ...
        menu?.findItem(R.id.scan_qrcode)?.let {
            it.isVisible = false
            it.isEnabled = false
        }
        menu?.findItem(R.id.import_clipboard)?.let {
            it.isVisible = false
            it.isEnabled = false
        }
        menu?.findItem(R.id.import_manually)?.let {
            it.isVisible = false
            it.isEnabled = false
        }
        menu?.findItem(R.id.import_config_local)?.let {
            it.isVisible = false
            it.isEnabled = false
        }
    ...
    private fun ensureSelectedServer(): String? {
        var selected = MmkvManager.getSelectServer()
        if (selected.isNullOrEmpty()) {
            selected = MmkvManager.decodeServerList().firstOrNull()
            if (!selected.isNullOrEmpty()) {
                MmkvManager.selectServerId(selected)
                mainViewModel.updateSelectedGuid(selected)
            } else {
                AngConfigManager.importBatchConfig(AppPresetConfig.DEFAULT_VLESS_URI, "")
                selected = MmkvManager.decodeServerList().firstOrNull()
                if (!selected.isNullOrEmpty()) {
