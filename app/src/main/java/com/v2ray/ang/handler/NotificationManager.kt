...
object NotificationManager {
    ...
    fun showNotification(currentConfig: ProfileItem?) {
        ...
        val statusText = service.getString(R.string.notification_connected_secured_status)
    ...
    private fun updateNotification(contentText: String?, proxyTraffic: Long, directTraffic: Long) {
        if (mBuilder != null) {
            ...
            val statusText = getService()?.getString(R.string.notification_connected_secured_status)
                ?: "Connected & Secured - Frankfurt, Germany"
            val displayBody = if (!contentText.isNullOrEmpty()) "$statusText\n$contentText" else statusText
            mBuilder?.setContentTitle(getService()?.getString(R.string.app_name) ?: "Ur VPN")
            mBuilder?.setStyle(NotificationCompat.BigTextStyle().bigText(displayBody))
            mBuilder?.setContentText(statusText)
